...
class DisturbPredictionLoss(AuxiliaryLoss):
    ...
    def get_aux_loss(
        ...
        **kwargs
        ...
        logits = logits.squeeze(-1)[:-1] # (T, B, 1) -> (T-1, B) final action does not have label
        ...
        avg_loss = (raw_focal_loss * loss_masks).sum() / torch.clamp(num_valid_losses, min=1.0)
        ...
        return (
            avg_loss,
            {
                ...
                "has_disturb_precision": cast(torch.Tensor, has_disturb_precision).item(),
    ...
    def inference(
        ...
        aux_model = actor_critic_output.extras[cls.UUID]["aux_model"] # given the trained model
